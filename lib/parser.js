...
var REGEXES = {
  ...
  recipient : [
    ...
    /^(.+?)\s?(?:\s\(.*?\))?\n?\s*[\[|<]mailto\:(.+?)[\]|>]/mi, // "john@acme.com<mailto:john@acme.com>" or "john@acme.com[mailto:john@acme.com]" or "john@acme.com (john@acme.com)<mailto:john@acme.com>"
    /^\<(.+?)\>:/m,
...
class Parser {
  ...
  __parseBounceDataError(str) {
    ...
    if (_permanent === true || (_temporary === false && _permanent === false)) {
      var _match = this.__regexes.temporary_safe.exec(str);
  ...
  __parseErrorData(str, code, label) {
    ...
    // Check if blocked
    var _blocked_match = this.__regexes.blocked.exec(str);
    ...
    // Check if spam
    var _spam_match = this.__regexes.spam.exec(str);
